...
//by any file in the project that has its requirements.
...
//why use dotenv here??
//because we need to use the environment variables in connectDB and we need to configure the dotenv file
...
.catch((err)=>{
     ...
     process.exit(1);
                    ...
                    //we donot use this as it makes the index.js here a bit clumsy
/*import express from "express";
const app=express();
//use js iffe to immediately invoke the function
//semicolon is used before it to prevent any errors
//syntax of iffe is (function(){})()
;(async()=>{
     try {
          //connect mongoose to the database
          await mongoose.connect(`${process.env.MONGODB_URI}/${DB_NAME}`);
          //connected the database along with its name
          //now we use app listeners for better error handling

          app.on("error",(error)=>{
               console.error("ERROR",error);
               throw(error);
          })//incase the express app is not able to talk to the connnected database

          //if no issue in connecting the database and app is able to listen to the port then->
